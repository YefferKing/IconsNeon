/**
 * Neon Icons - Auto Icon Loader
 * Permite usar iconos con la sintaxis: <i class="ni ni-icon-name neon-color"></i>
 * 
 * Uso:
 * 1. Incluir neon-icons.css
 * 2. Incluir neon-icons.js (este archivo)
 * 3. Usar <i class="ni ni-home neon-cyan"></i>
 */
...
(function() {
    ...
    const icons = {
        ...
        'fire': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8.5 14.5A2.5 2.5 0 0 0 11 12c0-1.38-.5-2-1-3-1.072-2.143-.224-4.054 2-6 .5 2.5 2 4.5 4 6.5 1.5 1.5 2 3.5 2 5.5a7 7 0 1 1-14 0c0-1.153.433-2.294 1-3a2.5 2.5 0 0 0 3.5 2.5z"/></svg>',
        ...
        'spotify': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8 15c2-1 4-1 6 0M7 12c3-1 5-1 8 0M6 9c4-1 7-1 11 0"/></svg>',
        
        // Finance & Business
        'wallet': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/><path d="M21 5a2 2 0 0 0-2-2h-2v4h4z"/><circle cx="16" cy="14" r="1"/></svg>',
        ...
        'file-minus': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="9" y1="15" x2="15" y2="15"/></svg>',
        ...
        'car': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><circle cx="17" cy="17" r="2"/></svg>',
        ...
        'toggle-left': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="5" width="22" height="14" rx="7" ry="7"/><circle cx="8" cy="12" r="3"/></svg>',
        'toggle-right': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="1" y="5" width="22" height="14" rx="7" ry="7"/><circle cx="16" cy="12" r="3"/></svg>',
        ...
        'octagon': '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"/></svg>',
    ...
    function initNeonIcons() {
        const iconElements = document.querySelectorAll('i.ni[class*="ni-"]');
        ...
        iconElements.forEach(el => {
            ...
            classes.forEach(cls => {
                if (cls.startsWith('ni-')) {
                    iconName = cls.replace('ni-', '');
                }
                if (cls.startsWith('neon-')) {
                    colorClass = cls;
                }
                if (['xs', 'sm', 'md', 'lg', 'xl', 'xxl'].includes(cls)) {
                    sizeClass = cls;
                }
            });
            
            if (iconName && icons[iconName]) {
                // Create span wrapper with neon-icon class
                const wrapper = document.createElement('span');
                wrapper.className = `neon-icon ${colorClass} ${sizeClass}`.trim();
                wrapper.innerHTML = icons[iconName];
                
                // Copy any additional styles
                if (el.style.cssText) {
                    wrapper.style.cssText = el.style.cssText;
                }
                
                // Replace the i element with the span
                el.parentNode.replaceChild(wrapper, el);
            } else if (iconName) {
                console.warn(`Neon Icons: Icon "${iconName}" not found`);
            }
        });
        
        console.log('🌟 Neon Icons initialized!');
    }

    // Run when DOM is ready
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initNeonIcons);
    } else {
        initNeonIcons();
    }
